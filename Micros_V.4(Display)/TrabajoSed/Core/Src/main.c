...
volatile uint8_t selectedMode = 1;      // 1: Económico, 2: Ajuste de temperatura
...
void handleEmergency(void) {
    // Parpadeo de ambos LEDs (rojo y verde) en el estado de emergencia
	 lcd_clear();
	 lcd_put_cur(0,0);
     lcd_send_string("<PAUSA DE EMERGENCIA>");
	 while (1) {
	        // Parpadeo de los LED
...
// Función para ejecutar el proceso según el modo seleccionado
/*
void executeProcessWithMode(void) {
	  lcd_clear();
      lcd_put_cur(0,0);
	  lcd_send_string("<<<<Selecciona modo>>>>");

	  if (HAL_GPIO_ReadPin(MODE_BUTTON_PORT, MODE_BUTTON_PIN) == GPIO_PIN_RESET) {
	        selectedMode = (selectedMode == 1) ? 2 : 1;  // Alternar entre los modos
    ...
    if (selectedMode == 1) {
    	...
    	lcd_clear();
        lcd_put_cur(0,0);
    	lcd_send_string("<<<<Modo Economico⬇️>>>>");
        while (1) {
            process();  // Ejecuta el proceso normal
            // Salir si se detecta cambio de modo
            if (HAL_GPIO_ReadPin(MODE_BUTTON_PORT, MODE_BUTTON_PIN) == GPIO_PIN_RESET) {
                selectedMode = 2;  // Cambiar al modo 2
                HAL_Delay(500);    // Debounce
                break;
            }
        }
    	...
    	lcd_clear();
    	        lcd_put_cur(0,0);
    	    	lcd_send_string("<<<<Modo Premium⬇️>>>>");;
        while (1) {
            process();  // Ejecuta el proceso normal
            currentTemperature = readTemperature();  // Leer temperatura del potenciometro


            // Salir si se detecta cambio de modo
            if (HAL_GPIO_ReadPin(MODE_BUTTON_PORT, MODE_BUTTON_PIN) == GPIO_PIN_RESET) {
                selectedMode = 1;  // Cambiar al modo 1
                HAL_Delay(500);    // Debounce
                break;
            }
        }
...
int main(void)
{
  ...
  while (1) {
         if (emergencyFlag) {
        	...
        	process();//executeProcessWithMode(); // Ejecutar el proceso principal /// INTERFAZ
         }

         HAL_Delay(100); // Pausa para evitar uso intensivo del procesador
     }
    /* USER CODE END WHILE */
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  /*Configure GPIO pin : BOTON_MODO_Pin */
  GPIO_InitStruct.Pin = BOTON_MODO_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(BOTON_MODO_GPIO_Port, &GPIO_InitStruct);
